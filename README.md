...
A tiny Windows 11 utility that embeds a live ping readout **inside the taskbar** (TrafficMonitor-style, not a tray icon): color-coded latency, a sparkline of recent pings, packet loss, and hover stats — a permanent replacement for keeping `ping google.com -t` open in a terminal.
...
- **Taskbar-embedded widget** next to the tray clock — on the primary taskbar, secondary-monitor taskbar(s), or all of them. It moves out of the way of other taskbar widgets (e.g. TrafficMonitor) automatically.
...
- Stack: C# / .NET 10 WinForms. The taskbar-embedding technique (`SetParent` into `Shell_TrayWnd` / `Shell_SecondaryTrayWnd`) is ported from [TrafficMonitor](https://github.com/zhongyang219/TrafficMonitor)'s Windows 11 code path.
